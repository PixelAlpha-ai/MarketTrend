...
from tensorflow.keras.layers import Conv2D, Conv1D, Flatten, Dense, Reshape
...
class InitialFeatureExtractor:
    def __init__(self, input_shape, num_initial_filters, kernel_size):
        self.model = Sequential([
            Conv2D(num_initial_filters, kernel_size, activation='relu', input_shape=input_shape, padding='valid')
        ])
...
if __name__ == "__main__":
    ...
    for num_initial_filters in num_initial_filters_list:
        feature_extractor = InitialFeatureExtractor(input_shape, num_initial_filters, kernel_size).get_feature_extractor()
        ...
        # Reshape the extracted features into the new image format
        train_features = train_features.reshape((train_features.shape[0], train_features.shape[3], train_features.shape[2]))
        val_features = val_features.reshape((val_features.shape[0], val_features.shape[3], val_features.shape[2]))
        ...
        for test_symbol in symbols:
            for test_timeframe in timeframe_list:
                ...
                test_features = test_features.reshape((test_features.shape[0], test_features.shape[3], test_features.shape[2]))
